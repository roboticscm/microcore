<script>
  ...
  $: isRendered = !view ? rendered : view.isRendered(btnType.startsWith('#') ? btnType.substring(1) : btnType, rendered);
  $: isDisabled = !view ? disabled : view.isDisabled(btnType.startsWith('#') ? btnType.substring(1) : btnType, disabled);
...
{#if isRendered}
  <button
    ...
    style={`${forceClass ? '' : isDense ? 'margin-left: 1px; margin-right: 1px; align-items: center; justify-content: center; padding-left: 0px; ' : '' }  ${style}`}
