...
const _StringUtil = class {
  ...
  static isEmpty(source) {
    if (typeof source !== "string") {
      return source === null || source === void 0;
    }
    return source === null || source === void 0 || source.trim().length === 0;
  }
  static replaceAll(source, find, replace) {
    if (_StringUtil.isEmpty(source)) {
      return "";
    }
  ...
  static replaceAlls(source, finds, replaces) {
    if (finds.length === 0 || replaces.length === 0 || _StringUtil.isEmpty(source) || finds.length !== replaces.length) {
      return source;
    }
    let replaceStr = _StringUtil.replaceAll(source, finds[0], replaces[0]);
    for (let i = 1; i < finds.length; i++) {
      replaceStr = _StringUtil.replaceAll(replaceStr, finds[i], replaces[i]);
    }
    return replaceStr;
  }
  ...
  static formatFTSParam(value) {
    if (_StringUtil.isEmpty(value)) {
      return "";
    }
    if (value.startsWith('"')) {
      return _StringUtil.replaceAll(value, " ", "<->");
    }
    if (value.startsWith("`")) {
      return _StringUtil.replaceAll(value, "`", "");
    }
    return _StringUtil.unaccentVietnamese(_StringUtil.replaceAll(_StringUtil.replaceAll(value, " ", ":*&") + ":*", "#", ""));
  }
  static formatSearchParam(value) {
    if (_StringUtil.isEmpty(value)) {
      return "%%";
    }
    return "%" + _StringUtil.replaceAll(_StringUtil.unaccentVietnamese(value), "'", "") + "%";
  }
  static formatExactlySearchParam(value) {
    if (_StringUtil.isEmpty(value)) {
      return "";
    }
    return _StringUtil.replaceAll(_StringUtil.replaceAll(_StringUtil.unaccentVietnamese(value), "'", ""), "#", "");
  }
...
__publicField(StringUtil, "unaccentVietnamese", (str) => {
  str = str.toLowerCase();
...
__publicField(StringUtil, "capitalize", (s) => {
  if (typeof s !== "string" || s.length === 0) {
    return "";
  }
  return s.charAt(0).toUpperCase() + s.slice(1);
});
__publicField(StringUtil, "snakeToCamelCase", (str) => str.replace(/([-_][a-z])/g, (group) => group.toUpperCase().replace("-", "").replace("_", "")));
__publicField(StringUtil, "removeMark", (str) => {
  if (!str || typeof str !== "string")
    return str;
  return str.replace(/<mark>/g, "").replace(/<\/mark>/g, "");
});
__publicField(StringUtil, "removeHtmlTag", (source) => {
  if (!source || typeof source !== "string")
    return source;
  return source.replace(/<\/?[^>]+(>|$)/g, "");
});
__publicField(StringUtil, "removeExtraSpace", (source) => {
  if (!source) {
    return "";
  }
  return source.replace(/\s+/g, " ");
});
__publicField(StringUtil, "splitHumanName", (fullName) => {
  if (!fullName) {
    return ["", ""];
  }
  fullName = _StringUtil.removeExtraSpace(fullName);
  const splits = fullName.split(" ");
  return [splits.slice(0, length - 1).join(" "), splits[splits.length - 1]];
});
__publicField(StringUtil, "getFirstWord", (source) => {
  if (_StringUtil.isEmpty(source)) {
    return source;
  }
...
__publicField(StringUtil, "getAvatar", (name) => {
  if (_StringUtil.isEmpty(name)) {
    return name;
  }
...
__publicField(StringUtil, "markStringSearch", (source, searchs, removeAccent = true) => {
  if (!source || !searchs) {
    return source;
  }
  if (typeof source !== "string") {
  ...
  let newSource;
  if (removeAccent) {
    newSource = _StringUtil.unaccentVietnamese(source);
    searchs = _StringUtil.unaccentVietnamese(searchs);
  } else {
    newSource = source;
  }
  for (let search of searchs.split("|")) {
    ...
    for (let i = indices.length - 1; i >= 0; i--) {
      source = _StringUtil.insertAt(source, `</mark>`, indices[i] + search.length);
      source = _StringUtil.insertAt(source, `<mark>`, indices[i]);
    }
    newSource = source;
  }
  return source;
});
__publicField(StringUtil, "normalizeSearchKeyword", (searchText) => {
  if (!searchText) {
    return "";
  }
...
__publicField(StringUtil, "replaceParamAsTag", async (stringSource, paramObj, disabled = false, textareaRows = 4, textareaCols = 100, namePrefix = "") => {
  if (!paramObj) {
    paramObj = {};
  }
  ...
  const rxp = /{{(.*)}}/g;
  let curMatch;
  const input = (name, value, id, forceDisabled, type = "text") => `<input autocomplete="off" ${type === "date" ? 'data-slots="dmyh"' : void 0} ${type === "date" ? 'placeholder="dd/mm/yyyy"' : void 0}  ${disabled || forceDisabled ? "disabled" : ""} class="report-margin-bottom" ${id ? 'id="' + id + '"' : ""} name="${name}" value="${value || ""}">`;
  ...
  while (curMatch = rxp.exec(stringSource)) {
    ...
    if (paramObj && Array.isArray(paramObj)) {
      ...
      newSource = html;
    } else if (param && param.trim().startsWith(HtmlPrefixParser.REPEAT)) {
      const uuid = genUUID();
      const onClick = `
    ...
    } else if (param && param.trim().startsWith(HtmlPrefixParser.REPEAT_MULTI)) {
      const uuid = genUUID();
      const onClick = `
          const parentElement = document.getElementById('${uuid}').parentElement.parentElement;
    ...
    } else if (param && param.trim().startsWith(HtmlPrefixParser.MULTI)) {
      const realParam2 = param.replace(HtmlPrefixParser.MULTI, "");
      const value = paramObj[realParam2] || "";
      ...
      const parts = _StringUtil.removeExtraSpace(param).split(" ");
      let indexField = 0;
      let helperFunction = void 0;
      if (parts.length > 1) {
        indexField = 1;
    ...
    } else if (param && param.trim().startsWith(HtmlPrefixParser.LINK)) {
      const realParam2 = param.replace(HtmlPrefixParser.LINK, "").trim();
      const value = paramObj[realParam2] || "";
      const html = `${value}`;
      newSource = newSource.replace(curMatch[0], html);
    } else if (param && param.trim().startsWith(HtmlPrefixParser.EACH_LOOP)) {
      ...
      if (isNestedEachBlock) {
        ...
        if (endNestedEachLoopIndex >= 0) {
          ...
          const normalizedContent = _StringUtil.transformToHandlesbarTemplate(replaceContent);
        ...
        const namePrefix2 = fieldName + ".";
        let html, blankHTML;
        if (paramObj && paramObj[fieldName]) {
          html = await _StringUtil.replaceParamAsTag(content, paramObj[fieldName], disabled, textareaRows, textareaCols, namePrefix2);
          blankHTML = await _StringUtil.replaceParamAsTag(content, {}, disabled, textareaRows, textareaCols, namePrefix2);
        } else {
          html = await _StringUtil.replaceParamAsTag(content, {}, disabled, textareaRows, textareaCols, namePrefix2);
          blankHTML = html;
        }
        const onAddClick = `
      ...
      if (fieldName.endsWith("__date__")) {
        ...
        html = input(namePrefix + _fieldName, value, void 0, false, "date");
      } else {
        if (fieldName.endsWith("__multi__")) {
          const value = paramObj[fieldName] || "";
...
__publicField(StringUtil, "replaceTagAsParam", (stringSource) => {
  let newSource = stringSource;
  let rxp, curMatch;
...
__publicField(StringUtil, "normalizeKey", (key) => {
